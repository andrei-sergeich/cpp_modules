...
void			Form::beSigned(const Bureaucrat& bur)
{
	...
	{
		_isSigned = true;
		std::cout << GREEN << bur.getName() << CYAN << " signed "
...
{
	if (!_isSigned)
		throw FormIsNotSignedException(std::string(RED + executor.getName() + " can't execute "
												   + this->getName() + " because form is not sign" + RESET).c_str());
	if (executor.getGrade() > _gradeToExecute)
		throw Bureaucrat::GradeTooLowException(std::string(RED + executor.getName() + " can't execute "
														   + this->getName() + " because below grade" + RESET).c_str());
}
...
Form::FormIsNotSignedException::FormIsNotSignedException(const std::string& errMsg) : _errMsg(errMsg) {}
